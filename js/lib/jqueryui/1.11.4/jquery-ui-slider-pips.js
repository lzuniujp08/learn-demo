...
(function($) {
        ...
        'float': function( settings ) {
            ...
            if ( options.handle ) {
                ...
                slider.element.find(".ui-slider-handle").each( function(k,v) {
                    $(v).append($tip[k]);
                    setLabelStyle($($tip[k]), slider.element);
                });
            }
            ...
            slider.element.on( options.event , function( e, ui ) {
                ...
                var _label = $(ui.handle).find(".ui-slider-tip");
                _label.html(options.formatLabel(val[0]));
                setLabelStyle(_label, this);
            });
    ...
    function setLabelStyle(_label, obj) {
        _label.removeClass("ui-slider-tip-left").removeClass("ui-slider-tip-right");
        var _p = $(obj).parent();
        var _pLeft = _p.offset().left,
            _pWidth = _p.width(),
            _lLeft = _label.offset().left,
            _lWidth = _label.width();
        var _left = _lWidth / 2;
        if (_lLeft < _pLeft) {
            _label.addClass("ui-slider-tip-left");
            _left = -5;
        }
        if ((_lLeft + _lWidth) > (_pLeft + _pWidth)) {
            _label.addClass("ui-slider-tip-right");
            _left = _lWidth - 5;
        }
        _label.css("margin-left", -_left + "px");
    }
